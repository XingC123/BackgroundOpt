 ...
 * @date 2024/7/15
 */
class RoleManagerServiceHook(
    classLoader: ClassLoader,
    runningInfo: RunningInfo,
) : IHook(classLoader, runningInfo) {
    override fun hook() {
        ClassConstants.RoleManagerService.afterConstructorHook(
            enable = OsUtils.isR,
            classLoader = classLoader,
            hookAllMethod = true
        ) { param ->
            val roleManagerService = RoleManagerService(param.thisObject)

            (runningInfo.packageManagerService as? PackageManagerServiceCompatUntilA11)?.let { pms ->
                pms.roleManagerService = roleManagerService
            }
        }
    }
}