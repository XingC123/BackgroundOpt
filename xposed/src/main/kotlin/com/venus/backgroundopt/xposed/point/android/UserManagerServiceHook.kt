 ...
 * @date 2024/10/26
 */
class UserManagerServiceHook(
    classLoader: ClassLoader,
    runningInfo: RunningInfo,
) : IHook(classLoader, runningInfo) {
    override fun hook() {
        ClassConstants.UserManagerService.afterConstructorHook(
            classLoader = classLoader,
            hookAllMethod = true
        ) {param->
            val userManagerServiceInstance = param.thisObject
            runningInfo.userManagerService = UserManagerService(userManagerServiceInstance)
        }
    }
}