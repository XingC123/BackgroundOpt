...
import com.venus.backgroundopt.common.util.preference.prefString
    ...
    private fun init() {
        ...
        val appItems = PackageUtils.getInstalledPackages(
            ...
            filter = if (isEnabledGlobalOomScore
                ...
                { packageInfo ->
                    !PackageUtils.isImportantSystemApp(packageInfo.applicationInfo)
                            || PackageUtils.isHasActivity(packageInfo)
                }
            }
        )
...
//        )?.let { appItems ->
        runOnUiThread {
            recyclerView.apply {
                ...
                adapter = ShowAllInstalledAppsAdapter3(this@ShowAllInstalledAppsActivity, appItems)
